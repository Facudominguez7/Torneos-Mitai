...
if (!empty($_GET['accion'])) {
    ...
    if ($_GET['accion'] == 'agregarTumbada') {
        ...
        if ($equipoLocal == $equipoVisitante) {
            ...
            $sqlVerificarPartido = "SELECT COUNT(*) AS count FROM tumbada
            ...
            if ($rowPartido['count'] > 0) {
                echo "<script>alert('El partido ya está programado.');</script>";
                
            } else {
                // Insertar el partido si los equipos están disponibles
                $sqlInsertarPartido = "INSERT INTO tumbada (idCategoria, idEquipoLocal, idEquipoVisitante, horario, cancha, idDia, idEdicion) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmtPartido = mysqli_prepare($con, $sqlInsertarPartido);
                mysqli_stmt_bind_param($stmtPartido, "iiisiii", $idCategoria, $equipoLocal, $equipoVisitante, $horario, $cancha, $dia, $idEdicion);
...
<section>
    <div class="flex items-center justify-center p-12">
        <div class="mx-auto w-full max-w-[550px]">
            <form action="index.php?modulo=agregar-tumbada&accion=agregarTumbada&idCategoria=<?php echo $idCategoria ?>&idEdicion=<?php echo $idEdicion ?>" method="POST">
                <div class="mb-5">
